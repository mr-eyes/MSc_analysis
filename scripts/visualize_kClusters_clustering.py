...
for line in sys.stdin:
    ...
    _threshold = re.findall(r"_(\d+)", line)[0]
    _value = re.findall(r":(\d+)", line)[0]
    _type = re.findall(r"\[_(\w+)\]", line)[0]
    _kmer_size = re.findall(r"k(\d+)", line)[0]

    if "t_" + _threshold not in kmers:
        kmers.append("t_" + _threshold)
...
layout = dict(title='<b>kClustering Biological Assessment | kSize = ' + _kmer_size + '</b>',
              xaxis = dict(title = '<b>kCluster Thresholds</b>'),
